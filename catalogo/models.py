...
class Autor(models.Model):
  ...
  # Se define __str__ para que sea legible tanto en el admin como en el shell
...
class Libro(models.Model):
    ...
    #Respuesta: Quiero evitar eliminar un autor si tiene libros asociados, para no perder esa información. Por eso se usa PROTECT en lugar de CASCADE.

    # ¿Por qué isbn debe ser único?
    #Respuesta: El ISBN es un identificador único para cada libro, esto establece que no puede haber dos libros con el mismo ISBN en el catálogo. Esto permite identificar de manera precisa cada libro y evitar confusiones.
    ...
    def tiene_disponibles(self) -> bool:
        ...
        La función tiene que devolver un booleano, no un número."""
...
class Prestamo(models.Model):
    ...
    #Respuesta: Un valor por defecto para fecha_prestamo sería la fecha actual, ya que en general se registra el préstamo en el momento en que se realiza. Por eso podríamos usar default=timezone.now para que se asigne automáticamente la fecha actual al crear un nuevo préstamo. Sin embargo, también podríamos dejarlo sin default para que el test lo defina explícitamente, dependiendo de cómo queramos manejar los casos de prueba y la flexibilidad que necesitemos en la creación de préstamos.
